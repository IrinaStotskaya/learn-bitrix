...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (!isset($arParams["PAGE_COUNT"]) || $arParams["PAGE_COUNT"] == "") {
    ...
    final public function executeComponent(): void
    {
        ...
        if ($this->cache->initCache(
            ...
            $arTypeAgents = self::getFieldListValue($arHlblock, 'UF_AGENT_ACTIVITY_TYPE'); // получить массив со значениями списочного свойства Виды деятельности агентов
         ...
         */ 
         $this->arResult['STAR_AGENTS'] = CUserOptions::GetOption('mcart_agents', 'options_star_agents');
        /*
    ...
    private static function getEntityDataClassById(array $arHlblock): string
    {
        ...
        $agents_data_class = $entity->getDataClass(); 
        return $agents_data_class;
    }
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
            $resEnum = CUserFieldEnum::GetList(array(), array("USER_FIELD_ID" => $fieldID,));
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $rsAgents = $entity::GetList([
             ...
             */
            "filter" => array("UF_AGENT_ACTIVITY" => 1),
            "count_total" => true,
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
            $activityType = $arTypeAgents[$arAgent["UF_AGENT_ACTIVITY_TYPE"]];
            $arAgent["UF_AGENT_ACTIVITY_TYPE_VALUE"] = $activityType;
            $arAgent["UF_AGENT_PHOTO_PATH"] = \CFile::getPath($arAgent["UF_AGENT_PHOTO"]);
    ...
    public function clickStarAction($agentID)
    {
         ...
         */
        $arStarAgents = CUserOptions::GetOption('mcart_agents', 'options_star_agents');
        if (isset($arStarAgents)){
            if (!is_array($arStarAgents)) {
                $arStarAgents = [$arStarAgents];
            }
            if (in_array($agentID, $arStarAgents)) {
                unset($arStarAgents[array_search($agentID, $arStarAgents)]);
            }
            else {
                $arStarAgents[] = $agentID;
            }
            $value = $arStarAgents;
        }
        else {
        ...
        $res = CUserOptions::SetOption('mcart_agents', 'options_star_agents', $value);
