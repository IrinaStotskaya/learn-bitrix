...
<?if($arResult["SHOW_EMAIL_SENT_CONFIRMATION"]):?>
	<div class="col-md-12">
	     <p class="font-weight-bold"><?echo GetMessage("AUTH_EMAIL_SENT")?></p>
    </div>
<?endif;?>

<?if(!$arResult["SHOW_EMAIL_SENT_CONFIRMATION"] && $arResult["USE_EMAIL_CONFIRMATION"] === "Y"):?>
	<div class="col-md-12">
	     <p class="font-weight-bold"><?echo GetMessage("AUTH_EMAIL_WILL_BE_SENT")?></p>
	</div>
<?endif?>
<noindex>
...
<form method="post" action="<?=$arResult["AUTH_URL"]?>" name="regform">
	<div class="row form-group">
        <div class="col-md-12 mb-3 mb-md-0">
		    <input type="hidden" name="SIGNED_DATA" value="<?=htmlspecialcharsbx($arResult["SIGNED_DATA"])?>" class="form-control"/>
			<span class="starrequired">*</span><label class="font-weight-bold"><?echo GetMessage("main_register_sms_code")?></label>
			<input size="30" type="text" name="SMS_CODE" value="<?=htmlspecialcharsbx($arResult["SMS_CODE"])?>" class="form-control" autocomplete="off" />
...
<form method="post" action="<?=$arResult["AUTH_URL"]?>" name="bform" enctype="multipart/form-data" class="p-5 bg-white border">
	...
	<div class="row form-group">
        <div class="col-md-12 mb-3 mb-md-0">
	        <p class="font-weight-bold"><?=GetMessage("AUTH_REGISTER")?></p>
        </div>
    </div> 
...
<?if($arResult["PHONE_REGISTRATION"]):?>
	<div class="row form-group">
        <div class="col-md-12">
		    <?if($arResult["PHONE_REQUIRED"]):?><span class="starrequired">*</span><?endif?><label class="font-weight-bold"><?echo GetMessage("main_register_phone_number")?></label>
...
<?if($arResult["USER_PROPERTIES"]["SHOW"] == "Y"):?>
	...
	<div class="row form-group">
		...
		<?if ($arUserField["MANDATORY"]=="Y"):?><span class="starrequired">*</span><?endif;?>
			<label class="font-weight-bold"><?=GetMessage("CHOOSE_USER_ROLE");?></label>
...
<?// ******************** /User properties ***************************************************
	...
	if ($arResult["USE_CAPTCHA"] == "Y")
	{
		...
		<div class="row form-group">
		    ...
		    <div class="col-md-12">
			    <span class="starrequired">*</span><label class="font-weight-bold"><?=GetMessage("CAPTCHA_REGF_PROMT")?></label>
		        <input type="text" name="captcha_word" class="form-control" value="" autocomplete="off">
	        </div>
	    </div>
		<?
	}
...
<div class="row form-group">
        <div class="col-md-12">
		    <a href="/login/auth.php" rel="nofollow"><b><?=GetMessage("AUTH_AUTH")?></b></a>
		</div>
</div>
